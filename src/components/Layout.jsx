        ...
        <div className="flex h-screen overflow-hidden bg-slate-50">
            ...
            <aside className="w-64 bg-white border-r border-slate-200 flex flex-col flex-shrink-0 z-20">
                <div className="p-6 border-b border-slate-100 flex items-center gap-3">
                    ...
                    <span className="font-bold text-lg text-slate-900 tracking-tight">Cerebro</span>
                </div>

                <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
                    <div className="px-3 pb-2">
                        <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Pipelines</span>
                    </div>
                    <NavItem to="/" icon={LayoutDashboard} label="Excel to KB" />
                    <NavItem to="/excel-db" icon={Database} label="Excel to DB" />

                    <div className="px-3 pb-2 pt-4">
                        <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Communication</span>
                    </div>
                    <NavItem to="/kb-chat" icon={MessageSquare} label="KB Chat" />
                    <NavItem to="/t2d-chat" icon={MessageSquare} label="T2D Chat" />
