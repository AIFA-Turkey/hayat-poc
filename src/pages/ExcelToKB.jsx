...
export const ExcelToKB = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.download_blob_url || !formData.doc_intel_api_key || !formData.kb_workspace_id) {
            setError('Please fill in required fields (Blob URL, Doc Intel API Key, and Workspace ID) before running.');
            return;
        }
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-6">
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-slate-900">Excel to KB</h1>
                    <p className="text-slate-500">Process Excel files and build a Cerebro Knowledge Base</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
                <Card title="Configuration" className="h-fit">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div>
                            <h4 className="text-xs font-semibold text-indigo-600 uppercase tracking-wider mb-3">Azure Blob Download</h4>
                            <Input label="Blob URL" name="download_blob_url" value={formData.download_blob_url} onChange={handleChange} placeholder="https://..." />
                            <Input label="Connection String" name="download_connection_string" value={formData.download_connection_string} onChange={handleChange} type="password" />
                        </div>

                        <div>
                            <h4 className="text-xs font-semibold text-indigo-600 uppercase tracking-wider mb-3">Data Prep</h4>
                            <div className="grid grid-cols-2 gap-4">
                                <Input label="Title Column" name="title_column" value={formData.title_column} onChange={handleChange} />
                                <Input label="URL Column" name="url_column" value={formData.url_column} onChange={handleChange} />
                            </div>
                        </div>

                        <div>
                            <h4 className="text-xs font-semibold text-indigo-600 uppercase tracking-wider mb-3">Azure Doc Intelligence</h4>
                            <Input label="API Key" name="doc_intel_api_key" value={formData.doc_intel_api_key} onChange={handleChange} type="password" />
                            <Input label="Endpoint" name="doc_intel_endpoint" value={formData.doc_intel_endpoint} onChange={handleChange} />
                        </div>

                        <div>
                            <h4 className="text-xs font-semibold text-indigo-600 uppercase tracking-wider mb-3">KB Builder</h4>
                            <Input label="KB Name" name="kb_name" value={formData.kb_name} onChange={handleChange} />
                            <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <h4 className="text-xs font-semibold text-indigo-600 uppercase tracking-wider mb-3">Azure Blob Upload</h4>
                            <Input label="Blob Name" name="blob_name" value={formData.blob_name} onChange={handleChange} />
